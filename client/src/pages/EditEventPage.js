...
const EditEventPage = () => {
  ...
  const fetchEvent = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Extract recurrence type from rule if present
        ...
        setFormData({
          ...
          start_time: event.start_time.slice(0, 5), // Convert to HH:MM
          end_time: event.end_time ? event.end_time.slice(0, 5) : '14:00',
  ...
  const handleChange = (e) => {
    ...
    setFormData(prev => {
      ...
      // If the start date changes and there's no end date, default end date to start date
      if (name === 'event_date' && !prev.end_date) {
        ...
        <form onSubmit={handleSubmit} className="create-event-form">
          ...
          <DateTimePicker
            ...
            onDateChange={(date) => setFormData(prev => {
              const newData = { ...prev, event_date: date };
              // If no end date is set, default it to the start date
              if (!prev.end_date) {
          ...
          <div className="form-group">
            ...
            />
            <small>Optional: Add a link for tickets, registration, or more info</small>
          </div>
